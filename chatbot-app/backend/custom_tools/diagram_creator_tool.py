...
A custom tool for creating various types of diagrams including AWS cloud diagrams and UML diagrams.
Optimized for server environments with proper error handling and direct image response.
"""
...
def is_headless_environment() -> bool:
    ...
    return (
        os.getenv('DISPLAY') is None and 
        platform.system() == 'Linux' and
        (os.getenv('CI') is not None or 
         os.getenv('DOCKER_CONTAINER') is not None or
         not shutil.which('xdg-open'))
    )


def safe_open_diagram(file_path: str) -> None:
    """Safe diagram opening that handles errors gracefully - COMPLETELY DISABLED"""
    # Completely disabled - never open diagrams in any environment
    logger.info(f"Diagram opening disabled for server environments: {file_path}")
    return
...
# Expose necessary functions and variables at module level
...
def save_diagram_to_directory(title: str, extension: str, content: str = None) -> str:
    """Helper function to save diagrams to the session-specific diagrams directory
    ...
    """
    # Use session-specific diagrams directory
    try:
        from utils.tool_execution_context import get_session_diagrams_dir
        diagrams_dir = get_session_diagrams_dir()
    except:
        # Fallback to global diagrams directory
        try:
            diagrams_dir = Config.DIAGRAMS_DIR
        except:
            diagrams_dir = os.path.join(os.getcwd(), "output", "diagrams")
...
def process_diagram_result(result_path: str, title: str) -> dict:
    """Process diagram result to return image bytes directly"""
    try:
        logger.info(f"Processing diagram result: {result_path}")
        
        # Check if file exists
        if not os.path.exists(result_path):
            return {
                "status": "error",
                "content": [{"text": f"Error: Generated diagram file not found at {result_path}"}]
            }
        
        # Read the image file and return bytes directly
        with open(result_path, 'rb') as f:
            image_bytes = f.read()
        
        # Determine format from file extension
        file_ext = result_path.lower().split('.')[-1]
        if file_ext == 'svg':
            format_type = 'svg'
        ...
        return {
            "status": "success",
            "content": [
                {"text": f"Diagram '{title}' created successfully."},
                {
                    "image": {
                        "format": format_type,
        ...
        logger.error(f"Error processing diagram result: {e}")
        return {
            "status": "error",
            "content": [{"text": f"Error processing diagram: {str(e)}"}]
        }
...
class DiagramBuilder:
    ...
    def _render_cloud(self, output_format: str) -> str:
        ...
        try:
            with CloudDiagram(name=self.title, filename=output_path, outformat=output_format, show=False):
                for node in self.nodes:
                    node_type = node.get("type", "EC2")
            ...
            # Skip opening diagram in server environments
            logger.info(f"Diagram created successfully: {output_file}")
            return output_file
        except Exception as e:
    ...
    def _render_graph(self, output_format: str) -> str:
        ...
        output_path = save_diagram_to_directory(self.title, "")
        rendered_path = dot.render(filename=output_path, format=output_format, cleanup=False, view=False)
        # Skip opening diagram in server environments
        logger.info(f"Graph diagram created successfully: {rendered_path}")
    ...
    def _render_network(self, output_format: str) -> str:
        ...
        plt.close()
        # Skip opening diagram in server environments
        logger.info(f"Network diagram created successfully: {output_path}")
        return output_path
...
class UMLDiagramBuilder:
    ...
    # ... (keeping all other UML diagram methods the same for brevity)
    # The rest of the UML methods remain unchanged from the original code
    ...
    def _save_diagram(self, dot: graphviz.Digraph, output_format: str) -> str:
        ...
        rendered_path = dot.render(filename=output_path, format=output_format, cleanup=False, view=False)
        # Skip opening diagram in server environments
        logger.info(f"UML diagram created successfully: {rendered_path}")
        return rendered_path

    # Simplified implementations for other UML types (keeping original logic)
        ...
        with dot.subgraph(name="cluster_0") as c:
            ...
            for element in self.elements:
                ...
                if elem_type == "component":
                    ...
                    dot.node(name, f"«interface»\\n{name}", shape="ellipse", style="dashed")

        for rel in self.relationships:
            rel_type = rel.get("type", "connection")
            if rel_type == "dependency":
                dot.edge(rel["from"], rel["to"], style="dashed", arrowhead="vee")
            else:
                dot.edge(rel["from"], rel["to"], arrowhead="none")
    ...
    def _render_profile(self, output_format: str) -> str:
        """Profile Diagram: UML profile extensions"""
        dot = self._create_dot_graph()
        for element in self.elements:
            name = element["name"]
    ...
    def _render_state_machine(self, output_format: str) -> str:
        """State Machine Diagram: State transitions and behaviors"""
        dot = self._create_dot_graph()
        for element in self.elements:
            name = element["name"]
    ...
    def _render_sequence(self, output_format: str) -> str:
        ...
        for i, element in enumerate(self.elements):
            ...
            ax.text(
                i, len(sorted_msgs) + 0.5, participant_labels[element["name"]], ha="center", va="center",
                fontweight="bold", fontsize=10,
            )
            ax.axvline(x=i, ymin=0, ymax=(len(sorted_msgs) + 0.2) / (len(sorted_msgs) + 1),
        ...
        for i, msg in enumerate(sorted_msgs):
            ...
            ax.text(mid_pos, y_pos + 0.1, label, ha="center", va="bottom", fontsize=9,
    ...
    def _render_interaction_overview(self, output_format: str) -> str:
        ...
        for element in self.elements:
            ...
            if elem_type == "initial":
                ...
                dot.node(name, "", shape="doublecircle", style="filled,bold", fillcolor="white", color="black", width="0.3")
                ...
                dot.node(name, name, shape="diamond", style="filled", fillcolor="white", color="black", width="1.5", height="1.5")
            elif elem_type == "fork":
    ...
    def _render_timing(self, output_format: str) -> str:
        """Timing Diagram: Time-based state changes"""
        if not self.elements:
            raise ValueError("At least one element is required for timing diagram")
        fig, ax = plt.subplots(figsize=(18, len(self.elements) * 2.5))
        y_ticks, y_labels = [], []
        for idx, element in enumerate(self.elements):
            ...
            for state_idx, state in enumerate(states):
                ...
                ax.broken_barh([(start, end - start)], (idx - 0.4, 0.8),
                              facecolors=color_cycle[state_idx % len(color_cycle)], edgecolor="black")
        ...
        plt.close()
        # Skip opening diagram in server environments
        logger.info(f"Timing diagram created successfully: {output_path}")
        return output_path
...
def create_diagram(
    ...
    edges: Optional[List[Dict[str, Union[str, int]]]] = None,
    elements: Optional[List[Dict[str, str]]] = None,
    relationships: Optional[List[Dict[str, Union[str, int]]]] = None,
    output_format: str = "png",
    style: Optional[Dict[str, str]] = None,
) -> Union[str, Dict[str, Any]]:
    """Create diagrams including AWS cloud diagrams and UML diagrams.

    This tool creates various types of diagrams and returns them as image data
    for proper integration with the frontend image rendering system.
    ...
    Returns:
        Dict[str, Any]: On success, returns a dict with status and content including image bytes.
    ...
    Examples:
        ...
        create_diagram(
            diagram_type="cloud",
            title="my_architecture",
            nodes=[
                {"id": "web", "type": "EC2", "label": "Web Server"},
                {"id": "db", "type": "RDS", "label": "Database"}
            ],
            edges=[{"from": "web", "to": "db", "label": "connects"}]
        )
        
        # UML Class Diagram
        create_diagram(
            diagram_type="class",
            title="user_order_model",
            elements=[
                {"name": "User", "type": "class", "attributes": "name: str\nemail: str", "methods": "login()\nlogout()"},
                {"name": "Order", "type": "class", "attributes": "id: int\ntotal: float", "methods": "calculate_total()"}
            ],
    ...
    try:
        ...
        # UML diagram types
        uml_types = [
            "class", "object", "component", "deployment", "package", "profile", "composite_structure",
            "use_case", "activity", "state_machine", "sequence", "communication", "interaction_overview", "timing"
        ]
        ...
        if diagram_type in uml_types:
            if not elements:
                return {
                    "status": "error",
                    "content": [{"text": "Error: 'elements' parameter is required for UML diagrams"}]
                }
            ...
            # Process the result
            processed_result = process_diagram_result(output_path, title)
            logger.info(f"UML {diagram_type} diagram created successfully: {output_path}")
            ...
            if not nodes:
                return {
                    "status": "error",
                    "content": [{"text": "Error: 'nodes' parameter is required for basic diagrams"}]
                }
            ...
            # Process the result
            processed_result = process_diagram_result(output_path, title)
            logger.info(f"{diagram_type} diagram created successfully: {output_path}")
            return processed_result
        ...
        return {
            "status": "error",
            "content": [{"text": f"Error creating diagram: {str(e)}"}]
        }

...
STACK_NAME="mcp-aws-pricing-server"
REGION="us-west-2"
...
if [ -n "$API_URL" ] && [ "$API_URL" != "None" ]; then
    echo -e "\n=== Testing AWS Pricing MCP server ==="
    ...
    echo -e "\n3. Testing getPricingServiceCodes:"
    curl -X POST $API_URL/mcp \
      -H "Content-Type: application/json" \
      -d '{
        "jsonrpc": "2.0",
        ...
        "method": "tools/call",
        "params": {
          "name": "getPricingServiceCodes",
          "arguments": {}
        }
      }'

    echo -e "\n4. Testing getPricingServiceAttributes for AmazonS3:"
    curl -X POST $API_URL/mcp \
      -H "Content-Type: application/json" \
      -d '{
        "jsonrpc": "2.0",
        ...
        "method": "tools/call",
        "params": {
          "name": "getPricingServiceAttributes",
          "arguments": {
    ...
    echo -e "\n5. Testing getPricing for AmazonS3:"
    curl -X POST $API_URL/mcp \
      -H "Content-Type: application/json" \
      -d '{
        "jsonrpc": "2.0",
    ...
    echo -e "\n6. Testing generateCostReportWrapper:"
    curl -X POST $API_URL/mcp \
      -H "Content-Type: application/json" \
      -d '{
        "jsonrpc": "2.0",
        ...
        "method": "tools/call",
        "params": {
          "name": "generateCostReportWrapper",
          "arguments": {
            "pricing_data": {
              "status": "success",
